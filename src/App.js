...
function App() {
  ...
  // Call the function with the example inputs and update the modified state
  React.useEffect(() => {
    const highlightedText = highlightHTMLContent(htmlContent, plainText, plainTextPositions);
    setModified(highlightedText);
  }, [htmlContent, plainText, plainTextPositions]);

  return (
    <div>  
      <p class = "paragraph">{modified}</p>
    </div>
  );
